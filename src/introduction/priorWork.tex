...
The desired mechanical properties were selected to mirror breast tissue based on additional team research~\cite{RefWorks:RefID:372-krakovskytumor}.
...
The senior capstone, while making strides in the early development of the device, had knowledge gaps that could benefit from additional testing, research, and refinement. This includes focusing on optimizing the manufacturing process, making the overall mesh thinner, decreasing the amount of barium sulfate embedded, and performing longer-term biodegradability testing.
...
\begin{figure}[p]
        \centering
        \BeginAccSupp{method=pdfstringdef,unicode,Alt={A diagram showing interactions between the device and its potential stakeholders within the overall system ecosystem.}}
        \includegraphics[width=0.9\textheight, angle=90, keepaspectratio]{../figs/introduction/ecosystem_mapping.png}
        \EndAccSupp{}
...
Given this change, the design objectives were modified from those of a patient-specific geometry to a uniform mesh with a thickness of 0.125mm. A thickness of 0.125mm was chosen because it was found to be the smallest reliably printable resolution that also provides substantial flexibility. The device was qualitatively stiff at thicker geometries such as 0.25mm. This mesh is shown below in Figure~\ref{fig:introduction:initialThinMeshDesign}. The minimal profile of this mesh allows it to be flexible and easily cuttable by surgeons. Additionally, triangular  cutouts were added to provide directional orientation for radiation oncologists when creating treatment plans. The mesh sheets were created in 20 x 20$mm^2$ and 38 x 38$mm^2$ squares to adequately cover traditional tumor sizes~\cite{RefWorks:RefID:371-bakhtardesign}.
...
The mesh design requirements included a need for the device to be fixed in place to address the concern of migration observed with existing devices like fiducial clips~\cite{RefWorks:RefID:344-mitchell2019adaptable}. The initial capstone team planned to secure the mesh to the tumor bed walls via suturing, though this could increase the implementation time substantially~\cite{RefWorks:RefID:372-krakovskytumor}. To address this adhesion concern, a texture was added to the top of the flat mesh to adhere to and increase the amount of friction between the mesh and tissue. Using the SolidWorks built in "Treadplate Bump" feature, a 3 x 3 mm treadplate bump with a texture offset of 0.75 mm and maximum element size of 0.125 mm was applied to the initial mesh~\cite{RefWorks:RefID:371-bakhtardesign}. This is shown below in Figure~\ref{fig:introduction:meshWithTreadplate}.
...
To evaluate the ability of the added surface texture to improve tissue adherence, a friction test was performed. This test evaluated a range of texture and mesh geometry parameters. The friction testing helped inform the device design by demonstrating that a higher offset height and larger treadplate features yielded a higher static coefficient of friction.
...
Initial powder-based PLCL extrusion testing was conducted on a Felfil EVO and Filabot EX6. The goal was to create a 3D printable PLCL filament which would make the overall implant biodegradable. Eventually, barium sulfate, a contrast agent, would be incorporated within this extrusion process to improve the radiopacity of the device. It was found that these powder-based extrusions yielded overly brittle and thin filament which could not be reliably 3D printed.

Nomisma 70/30 PLCL powder was used for this extrusion testing~\cite{RefWorks:RefID:387-nomisma}. These two tabletop extruder systems are shown below in Figure~\ref{fig:introduction:felfilAndFilabotExtruders}.
...
There is recommended research to be conducted following the research conducted in Adrian Bakhtar's master's thesis. This includes interviewing additional stakeholders such as radiation oncologists to better understand the device requirements from an imaging perspective, exploring fabrication and manufacturing techniques of PLCL with and without a radiopaque agent, studying effects on interobserver variability using this device, and evaluating effects of surface texture on biocompatibility~\cite{RefWorks:RefID:371-bakhtardesign}.

This work would further the development of the device, allowing it to be created in-house rather than externally sourced as well as incorporating biodegradability and radiopacity functionality into the device. Once an initial model of the device is created, however, interviews and interobserver variability studies should be conducted to determine the efficacy of the device and whether additional iterations are required.

\subsection{Undergraduate Thesis\label{sec:introduction:priorWork:undergradThesis}}
The author defended an undergraduate thesis on the early stage development of this research in 2024. This thesis explored the mechanical properties of externally sourced PLCL filament to eventually compare to custom-made PLCL filament produced by the research team~\cite{RefWorks:RefID:370-einsteinisaac, RefWorks:RefID:42-latticemedical}. Comparing the mechanical properties of the custom-made PLCL filament to commercially available filament validates the manufacturing process and ensures custom-made filament will behave similarly to commercially used filament.
