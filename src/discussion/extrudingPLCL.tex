\section{Extruding PLCL Discussion\label{sec:discussion:extrudingPLCL}}

\subsection{Qualitatively Assessing Filament Output\label{sec:discussion:extrudingPLCL:assessingFilamentOutput}}

"Good quality" filament output is determined through various quantitative and qualitative factors. A detailed explanation of these factors can be found in~\fullref{sec:literatureReview:extrusion:issues}.

\subsubsection{Quantitative Factors\label{sec:discussion:extrudingPLCL:assessingFilamentOutput:quantitative}}

Quantitatively, filament thickness and screw current are the predominant metrics used to assess filament quality.

\paragraph*{Filament Thickness}
Filament thickness should stay within $1.75mm \pm 0.1mm$. This metric indicates steady filament output, and consistent filament thickness is required for reliable 3D printing.

\paragraph*{Extruder Current}
Extruder current should also remain relatively steady and never exceed 6000mA. Rapid rises in current are indicative of blockages forming within the extruder barrel~\cite{RefWorks:RefID:396-3devotroubleshooting}. Adequate and unusable filament based on extruder current and filament thickness are shown below in Figure~\ref{fig:discussion:extrudingPLCL:quantitativeFactors}.

\begin{figure}[h!]
        \centering
        \includegraphics[width=\linewidth]{../figs/discussion/plclExtrusions/filament_assessment_quantitative.png}
        \caption{Quantitatively assessing filament thickness. Unusable (top) and usable filament (bottom)~\cite{RefWorks:RefID:396-3devotroubleshooting}.}
        \label{fig:discussion:extrudingPLCL:quantitativeFactors}
\end{figure}

\subsubsection{Qualitative Factors\label{sec:discussion:extrudingPLCL:assessingFilamentOutput:qualitative}}

Qualitatively, filament output rate, filament surface texture, and circularity of filament are some of the factors assessed to determine "good quality" filament. Many of these factors were determined and identified through experience and discussions with 3Devo customer support.

\paragraph*{Filament Output}
Thin filament output indicates inadequate back pressure pushing material out of the extruder which can be caused by blockages or insufficient material flow.

\paragraph*{Filament Surface}
Filament surface texture, specifically filament being rough or bumpy, is often indicative of contaminants in the barrel either getting embedded in the filament output or scratching/gouging the extrusion output.

\paragraph*{Filament Circularity}
Flat or non-circular filament can mean the output is not being properly cooled before moving through the filament sensor. This can require increased cooling by increasing fan speed or slowing the output rate.

Some examples of poor filament quality noticed throughout PLCL extrusions can be found below in Figure~\ref{fig:discussion:extrudingPLCL:qualitativeFactors}.

\begin{figure}[h!]
        \centering
        \includegraphics[width=\linewidth]{../figs/discussion/plclExtrusions/filament_assessment_qualitative.png}
        \caption{Qualitatively assessing filament thickness. Thin output (A), rough surface texture (B), flat filament (C).}
        \label{fig:discussion:extrudingPLCL:qualitativeFactors}
\end{figure}
...
Lastly, the filament had air bubbles inside indicative of temperature being too high. If the temperature is too high, the powder is likely degrading into a gas~\cite{RefWorks:RefID:396-3devotroubleshooting}. These air bubbles are shown below in~\autoref{fig:discussion:extrudingPLCL:powderExtrusion:airBubbles}
...
\begin{figure}[h!]
        ...
        \label{fig:discussion:extrudingPLCL:powderExtrusion:airBubbles}
\end{figure}
...
Also, the process of extruding powder was found to be inefficient as it was difficult to keep the powder from dispersing when transferring from the initial container to the hopper. This led to dirtying of equipment and wasted raw material.

\paragraph*{Key Takeaways}
% order feeder, powders will always be brittle
Key takeaways from this extrusion experiment are that: a 3Devo Feeder~\cite{RefWorks:RefID:310-3devofeeder} should be ordered to address clumping and variable flow rate, heat zones should be decreased to address clumping, cleaning supplies such as wipes and a vacuum should be ordered to address powder dispersion, and alternative extrusion routes should be explored to address inherent brittleness of powder extrusions.
...
It was determined that the College of Pharmacy did not have adequate equipment to assist in mixing PLA and PCL pellets. Any mixing equipment they were able to provide was meant for small-scale (less than 5 grams) mixing.
...
After discussing the option of chemically combining the pellets with the PSRF (see~\fullref{methodology:externalLabs}) using chloroform or DCM as a dissolving agent, the team was told this could not be done. For safety purposes, the PSRF no longer carries DCM or chloroform and is not allowed to work with these materials in most cases.
...
To address this bottleneck, Dr. Mulyana suggested the team use a twin screw extruder in the ISE department. While this equipment would mix materials well, it would require high volumes of raw materials to run and time to learn how to use this equipment. It was decided that a twin screw extruder would be a viable backup option if using a single screw extruder did not mix materials well enough.
...
While the filament thickness was variable and overall too thin, this extrusion achieved a notable milestone for the research team. This was the first time the research team was able to steadily extrude good-quality PLCL filament (no air bubbles and not overly brittle).

This extrusion proved the efficacy of the starve feeder given the improvements in extrusion output rate and lack of clogging from the initial PLCL extrusion (see~\fullref{sec:methodology:extrudingPLCL:pelletExtrusions:initialExtrusion}).
...
Suggestions from 3Devo customer support included lowering heat zones 4 and 3 and raising extruder screw RPM. Neither of the suggestions resolved the filament thickness variability, and 3Devo customer support had limited additional suggestions.

Because the heat zone adjustment was not yielding adequate results, 3Devo customer support suggested more routinely purging the extruder in case contaminants were blocking the flow of material. As a result, purging compounds were explored (see ~\fullref{sec:methodology:extrudingPLCL:purgingExtruder}) and a smaller nozzle was ordered from 3Devo~\cite{RefWorks:RefID:392-3devo}.

3Devo customer support also suggested replacing the standard 4mm nozzle with a 2mm nozzle to produce initial output at a filament thickness which is closer to the ideal 1.75$mm$ value. Since the nozzle had to be ordered from 3Devo which required international shipping, it was assumed this would have a substantial lead time. Therefore, alternative extrusion options were explored while waiting for a smaller nozzle to arrive (see ~\fullref{sec:methodology:extrudingPLCL:felfilSystem}).
