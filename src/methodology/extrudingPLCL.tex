\section{Extruding PLCL\label{sec:methodology:extrudingPLCL}}

\subsection{Summary of PLCL Extrusions\label{methodology:extrudingPLCL:plclSummary}}

A significant portion of the research in this thesis pertains to extruding PLCL into a 3D printable filament to develop a biodegradable implant. Multiple approaches that can be utilized to fabricate an implantable device were employed to eventually achieve this outcome. Powder extrusions were first attempted as PLCL was only available in a powder form. It was found that powder extrusions yielded unprintably thin and brittle output. As a result, this was followed by manually and then automatically combining PLA and PCL pellets in a single screw extruder as pellets are the preferred form factor for extruding materials. Two extruders, 3Devo Filament Maker 1 and the Felfil Evo, were used to conduct these extrusions. When pellets alone yielded overly thin filament, shredding or regrinding the initial PLCL output and re-extruding was tested to address thickness tolerance concerns. Figure~\ref{methodology:extrudingPLCL:plclSummary} outlines this process at a high level.
...
Based on material availability, the team attempted to extrude PLCL powder to create PLCL filament. While pellets are more commonly extruded than powders (see~\fullref{sec:literatureReview:extrusion:powder}), the team was unable to source PLCL pellets at a scalable price. As a result, PLCL powder was bought from a company called Nomisma~\cite{RefWorks:RefID:387-nomisma} for these initial extrusions on a 3Devo Filament Maker 1 single screw extruder.

Prior to extruding, the PLCL powder was dried in a vacuum oven at 45\textcelsius ~overnight based on the material manufacturer's recommendations. The Polymer Synthesis Research Facility (PSRF) at Ohio State University was utilized for this task (see~\fullref{methodology:externalLabs} for more information on research conducted with this additional lab).
...
Discussion of these findings can be found in~\fullref{sec:discussion:extrudingPLCL:pelletExtrusions:combiningMaterials}.
...
In this extrusion, the 3Devo Feeder was used for the first time. The temperature profile used is shown below in Table~\ref{tab:methodology:extrudingPLCL:initialPelletExtrusion:tempPresets} based on existing literature~\cite{RefWorks:RefID:253-åkerlund2022effect}. All pellets were poured into the hopper with the Feeder running and the extrusion began.
...
The highlights of this extrusion can be found below in Table~\ref{methodology:extrudingPLCL:pelletExtrusions:usingStarveFeeder:initialPLCLExtrusion}.
...
After performing a Devo Mid-Temp (MT) purge (see ~\fullref{sec:methodology:extrudingPLCL:purgingExtruder}), multiple other PLCL pellet-based extrusions were attempted as detailed in ~\autoref{tab:appendix:extrusionSummary}.

\paragraph*{Adjusting Heat Zones and RPM}
...
A final extrusion was attempted following a published methodology that used a 3Devo Filament Maker One but pre-combined PLA/PCL pellets. This paper set heat zones to 140, 155, 160, and 160 (all \textcelsius) for heaters 4 through 1 respectively~\cite{RefWorks:RefID:253-åkerlund2022effect}.
...
Based on the thin filament output by the 3Devo Filament Maker One when extruding a PLA/PCL blend (see ~\fullref{sec:methodology:extrudingPLCL:pelletExtrusions:usingStarveFeeder:addressingFilamentThickness}), the Felfil Evo was tested for PLCL extrusions.

The Felfil Evo had been used initially for powder-based PLCL extrusions, but research was paused due to unsuccessful results (see ~\fullref{sec:introduction:priorWork:otherTeamWork:plclExtrusions}). Since the 3Devo Filament Maker One, a more robust extruder, also was unsuccessful extruding powders, it was hypothesized that powder-based extrusions for any extruder may be challenging and the Felfil Evo may be more effective with pellet-based extrusions.
...
Following the successful PLA pellet-based extrusion on the Felfil Evo (see Section~\fullref{sec:methodology:extrudingPLCL:felfilSystem:felfilEvoInitialTesting}), a PLA/PCL extrusion was attempted using the same methodology as the 3Devo PLA/PCL extrusions (see Section~\fullref{sec:methodology:extrudingPLCL:pelletExtrusions:usingStarveFeeder:initialPLCLExtrusion}).
...
Regardless, various materials were researched to assess the viability of this option. Some of these materials are shown below in Figure~\ref{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:siliconHopperCoating}
...
To quickly develop a method of insulating the metal hopper wall from the extruding material, a 3D printed spacer was designed.

To model the component, an assembly of the hopper was created in SolidWorks using CAD files provided by Felfil. A custom shape was created to press-fit along the walls of the hopper. A thin piece was 3D printed to ensure proper tolerances.

Once it was confirmed that the geometry of the component fits within the available space, a thicker piece was designed to increase distance between the metal wall and extruder material.

While the 3D printed component fit well alongside the hopper, as material was extruded it began pushing against this piece and disloging it from the wall. This led to material clumping between the newly formed wall and insulator gap. To address this, a magnet was embedded in the insulator piece to strongly secure it to the metal wall and prevent material from traveling in between. The development of the hopper insulator is shown below in Figure~\ref{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:hopperInsulator}

\begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\linewidth]{../figs/methodology/plclExtrusions/pelletExtrusions/felfilEvo/hopper_insulator.png}
        \caption{Felfil Evo hopper without insulation (A) and with insulator pieces (B).}
        \label{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:hopperInsulator}
\end{figure}
...
A Felfil Shredder was used to shred existing prints and filament to create regrind for second-pass PLCL extrusions. Initially, first-pass PLCL filament was cut and fed into the shredder. This was re-fed into the shredder multiple times to create semi-uniform sized pieces of regrind. Even after multiple passes, however, there were still thin strands that were too small to be collected by the shredder blades.

The shredding process is shown below in Figure~\ref{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:shreddingFilament}

\begin{figure}[h!]
        \centering
        \includegraphics[width=0.7\linewidth]{../figs/methodology/plclExtrusions/pelletExtrusions/felfilEvo/shredder_process.png}
        \caption{Felfil Shredder process. Initial cut filament (A) and shredder output after four passes (B).}
        \label{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:shreddingFilament}
\end{figure}

It was found that shredding thicker pieces (roughly $2.85mm$) yielded similar shredding results after two passes rather than four.

\paragraph*{Filtering Regrind By Size}

A sieve designed by Felfil was downloaded from their online CAD file repository~\cite{RefWorks:RefID:495}. This was designed to sit on top of the shredder output drawer and filter materials that were small enough to be re-extruded (roughly $5mm$ by $5mm$). This sieve was found to be difficult to use as thin long strands could still pass between the openings. A picture of the sieve is shown below in Figure~\ref{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:shredderSieve}

\begin{figure}[h!]
        \centering
        \includegraphics[width=0.5\linewidth]{../figs/methodology/plclExtrusions/pelletExtrusions/felfilEvo/shredder_sieve.png}
        \caption{3D printed Felfil Shredder sieve.}
        \label{fig:methodology:extrudingPLCL:pelletExtrusions:felfilEvo:shredderSieve}
\end{figure}
...
Following the above adjustments (hopper insulation, filament thickness tracking, and shredding testing), multiple PLA/PCL pellet extrusions were performed on the Felfil system.

The extrusion temperature was adjusted between 160 and 170\textcelsius, and the RPM was adjusted between 4 and 8RPM. The spooler parameters were also manually adjusted to assess their impact, but were then left on the "Soft Mode" presets.

Results and discussion of these first and second-pass extrusions can be found in~\fullref{sec:results:extrudingPLCL:pelletExtrusions:felfilEvo:modifiedSystem} and~\fullref{sec:discussion:extrudingPLCL:pelletExtrusions:felfilEvo:modifiedSystem} respectively.
