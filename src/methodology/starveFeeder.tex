...
To automate and standardize the starve feeding process (see~\fullref{sec:methodology:extrudingPLCL:pelletExtrusions:usingStarveFeeder:initialPLCLExtrusion}), an automatic pellet dispensing starve feeder was developed. This was adapted from an online model and went through various hardware and user interface (UI) iterations. Starve feeding was automated to increase efficiency and reliability when extruding materials that require starve feeding.
...
In addition to belt tensioning, Dr. Hong was able to find an old drivetrain system that was powered by a Nema 23 stepper motor. This system was disassembled and the stepper motor pulley, large gear, and rubber belt was used in the 3D printed starve feeder. These metal, plastic, and rubber components replaced the 3D printed counterparts.
...
With this initial image, a software called ImageJ was used to measure the slot dimensions. ImageJ works by marking a reference distance, mapping that distance to a pixel conversion, and then estimating measurements of lines drawn on a photo. The calipers in the initial photo were used for a reference measurement.
...
The final Arduino-based code can be found in ~\fullref{sec:appendix:pelletDispenserCode:Arduino}.
...
With the GUI in place, the keypad component was removed from the system. The final Python-based code for the GUI can be found in ~\fullref{sec:appendix:pelletDispenserCode:Python}.
