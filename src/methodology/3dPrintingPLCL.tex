...
All 3D printing was performed on a Bambu P1S. Printing of PLCL was researched to be able to 3D print the final device implant from a custom-made PLCL filament.

\subsection{3D Printing Lattice PLCL\label{sec:methodology:3dPrintingPLCL:latticePLCL}}

To begin testing 3D printability of PLCL filament, externally sourced Lattice Medical PLCL filament was used~\cite{RefWorks:RefID:204-70}. This is a commercially produced filament which could therefore act as a benchmark and starting point for testing the printability of custom PLCL filament. However, with no contrast agent, this filament alone would not be adequately discernible via imaging.
...
Due to the repetitive clogging caused by the custom-made PLCL filament, a clog cleaning procedure was developed. While Bambu recommends initial actions to resolve a clog such as moving a pin through the nozzle, these suggestions were ineffective when trying to remove PLCL filament from the hotend.
