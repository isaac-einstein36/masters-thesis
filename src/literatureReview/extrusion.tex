...
Literature on extrusions was crucial for understanding and performing extrusion research throughout this thesis. PLCL was extruded and PLA with barium sulfate was extruded as described in~\fullref{sec:methodology:extrudingPLCL} and~\fullref{sec:methodology:extrudingBaSO4} respectively. The literature described in this section was important in understanding adjustable parameters and troubleshooting when performing extrusions.

\paragraph*{Overview of Extrusions}
...
Various issues when extruding filament were encountered when extruding PLCL and PLA with barium sulfate. Better understanding the causes and remedies of these issues allows for a more efficient extrusion research session.
...
Regrind was created and extruded when attempting to refine the filament thickness of extruded PLCL, thus requiring research of existing literature (see~\fullref{sec:methodology:extrudingPLCL:felfilSystem:felfilShredder}).
...
Using recycled filament can lead to a decrease in mechanical properties such as tensile strength or strain at break~\cite{RefWorks:RefID:218-lee2018development,RefWorks:RefID:470-boughanmi2024repetitive}. The results of one study illustrating the changes in mechanical properties of regrind are shown below in Figure~\ref{fig:literatureReview:regrind_changes_in_mechanical_properties}
...
Powder extrusions were performed with initial PLCL extrusions that used PLCL powder purchased from a materials company called Nomisma (see~\fullref{sec:methodology:extrudingPLCL:powderExtrusion}).
...
Purging an extruder was researched when filament output issues arose extruding PLA/PCL blends. Purging was also important when extruding PLA with barium sulfate to eliminate any residual powder in the extruder.

Various purging compounds were researched and tested to find the optimal compound for the materials used in this research (see~\fullref{sec:methodology:extrudingPLCL:purgingExtruder}).
